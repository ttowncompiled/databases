...
FROM Users AS any_user
    ...
    ) AS agg_female_friends ON any_user.uid = agg_female_friends.uid1
    LEFT JOIN (
    ...
    ) AS agg_male_friends ON any_user.uid = agg_male_friends.uid1;
...
FROM Users LEFT JOIN (
  ...
  FROM Comments
      INNER JOIN Users AS males ON Comments.recipient = males.uid
      INNER JOIN Users AS females ON Comments.poster = females.uid
  WHERE males.gender = 'M' AND females.gender = 'F'
) AS no_females ON Users.uid = no_females.uid
